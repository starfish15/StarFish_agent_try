...
补充：Agent 会在进程内保留历史消息，跨轮对话会一起进入上下文。
当上下文字符数超过 `MAX_CONTEXT_CHARS` 时，会自动摘要压缩早期消息，
并保留最近 `CONTEXT_KEEP_LAST` 条消息。

示例：验证是否触发压缩
1) 启动前设置：`DEBUG_CONTEXT_COMPRESSION=1`，并将 `MAX_CONTEXT_CHARS` 调小（如 300）
2) 连续输入多轮较长文本
3) 控制台会出现类似日志：`Context compressed: summary_chars=..., kept_messages=..., total_messages=...`
...
上下文与摘要：
- `MAX_CONTEXT_CHARS`：超过该字符数触发摘要压缩
- `CONTEXT_KEEP_LAST`：压缩时保留最近 N 条消息
- `SUMMARY_MAX_CHARS`：摘要最大字符数
- `DEBUG_CONTEXT_COMPRESSION`：压缩触发时打印日志（1/true/yes 开启）
